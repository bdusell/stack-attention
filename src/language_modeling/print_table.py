...
def read_test_data(path):
    scores_path = path / 'eval' / 'dyer-ptb-test' / 'scores.json'
    with scores_path.open() as fin:
        return json.load(fin)

def get_test_perplexity(cache):
    best_trial = cache['best_trial']
    if best_trial:
        return read_test_data(best_trial.path)['perplexity']
    else:
        return None
...
def main():
    run_main(
        columns=[
            ...
            Column('Val.', 'c', 'val_perplexity', format_float(places=2), bold_min=True),
            Column('Test', 'c', 'test_perplexity', format_float(places=2), bold_min=True)
        ],
        callbacks={
            ...
            'test_perplexity' : get_test_perplexity
        }
    )
